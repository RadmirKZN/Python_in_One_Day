class Game:
    ...
    def noOfQuestions(self, value):
        if value < 1:
            ...
            print("Hence, number of questions will to 10 ")
        else:
            self._noOfQuestions = value

class BinaryGame(Game):
    def generateQustion(self):
        from random import randint
        score = 0

        for i in range(self.noOfQuestions):
            base10 = randint(1, 100)
            userResult = input ("\nPlase convert %d to binary:" %(base10))
            ...
            while True:
                try:
                  ...
                  if answer == base10:
                    ...
                    print("You did no enter a binary number. Please try again.")
...
class MathGame(Game):
    def generateQuestion(self):
        ...
        symbolList = ['', '', '', '']
        operationDict = {1:' + ', 2:' - ', 3:'*', 4:'**'}
        for i in range(self.noOfQuestions):
          for index in range(0, 5):
            numberList[index] = randint(1, 9)
          for index in range(0,4):
            if index > 0 and symbolList[index -1] == '**':
                symbolList[index] = operationDict[randint(1, 3)]
            else:
                symbolList[index] = operationDict[randint(1, 4)]
        ...
        while True:
            try:
              ...
              print("You did not enter a valid nuber. Please try again.")
              userResult = input("\nPlease evaluate %s:" %(questionString))
